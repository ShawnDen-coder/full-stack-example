...
export function createAuthModule(options: AuthModuleOptions): AuthModule {
  ...
  const sessionFor = async (request: Request) =>
    (await (auth.api as any).getSession({ headers: request.headers })) as any;
  return {
    auth,
    guards: {
      ...
      requireTenant: async (context, next) => {
        ...
        const [membership, tenant] = await Promise.all([
          options.database.query.member.findFirst({
            where: and(eq(member.organizationId, tenantId), eq(member.userId, value.user.id)),
          }),
          options.database.query.organization.findFirst({
            where: eq(organizationTable.id, tenantId),
          }),
        ]);
        if (!membership) return context.json({ error: "Forbidden" }, 403);
        if (!tenant || tenant.status !== "active")
          return context.json({ error: "Organization disabled" }, 403);
        context.set("tenantId", tenantId);
        context.set("memberRole", membership.role);
        context.set(
          "principal",
          createTenantPrincipal({
            session: createSessionPrincipal({
              userId: value.user.id,
              sessionId: value.session.id,
              platformRoles: value.user.role === "platform-admin" ? ["platform-admin"] : ["user"],
            }),
            tenantId,
      ...
      requirePermission: (requirement) => async (context, next) => {
        const value = await sessionFor(context.req.raw);
        const role =
          value?.user?.role === "platform-admin"
            ? "owner"
            : ((context.get("memberRole") as "owner" | "admin" | "member" | undefined) ?? "member");
        const allowed =
    ...
    platform: {
      ...
      async requestPasswordReset(input) {
        await (auth.api as any).requestPasswordReset({
          body: { email: input.email, redirectTo: `${options.baseURL}/reset-password` },
        });
        await options.securityEvents?.emit({
