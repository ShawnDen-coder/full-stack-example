...
服务端还提供 `getOpenApiDocument()`，只返回产品实际使用的认证和工作区端点；API 宿主将它合并到 `/openapi.json`。登录、注册标记为公开，其余 Session、退出和工作区操作使用同一个 `apiKeyCookie` Session Cookie scheme，不宣称支持 Bearer Token。
...
`setupAuthApp()` 在传入的 Hono app 上挂载相对 `/auth/*` 和 `/platform/*` 路由。API 组合根把认证子应用与 Todos、Jobs Admin、Diagnostics 一起挂载到 `/api`，所以公开 URL 仍是 `/api/auth/*` 和 `/api/platform/*`。
...
`BETTER_AUTH_URL`（API/Auth 地址）、`WEB_ORIGIN`（浏览器跳转地址）、`BETTER_AUTH_SECRET`（至少 32 字符）和 `DATABASE_RUNTIME_URL`（运行时连接）。Auth 不使用 migrator 连接；本地与生产均应显式提供 runtime URL。

初始化管理员由独立的一次性 `admin-provision` 进程执行。它使用 `DATABASE_RUNTIME_URL` 与 `PLATFORM_ADMIN_EMAIL`、`PLATFORM_ADMIN_NAME`、`PLATFORM_ADMIN_PASSWORD`；缺失账号会被创建，同邮箱账号会被幂等提升为 `platform-admin`，现有密码不会被覆盖。API 长期运行时不接收这些管理员初始化凭据。

Auth server 运行在 API 和 provisioning 进程中，共用注入的 runtime Database 与 `BETTER_AUTH_SECRET`；浏览器只加载 Auth client。Auth 不创建连接池，也不持有 migrator 凭据。

## 生命周期

登录创建 Better Auth Session；每个租户请求由服务端检查 Session、活动组织、成员关系、组织状态和资源权限，再把已验证的 tenant principal 交给业务路由。平台管理员初始化只在 provisioning 命令运行时执行；平台管理员身份本身不会授予任何组织内的业务权限。
...
PostgreSQL 集成测试不会被普通 `just test` 自动触发。配置 `DATABASE_RUNTIME_URL` 并启动数据库后，使用 `just auth-test-integration` 显式运行用户、组织、登录、Session 和管理员 provisioning 测试。
