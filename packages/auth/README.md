...
初始化数据库时可设置 `PLATFORM_ADMIN_EMAIL`、`PLATFORM_ADMIN_NAME` 和 `PLATFORM_ADMIN_PASSWORD`。数据库 migration 完成后 API 会创建缺失的账号，或将同邮箱账号幂等提升为 `platform-admin`；已存在账号的密码不会在 API 启动时重置。
