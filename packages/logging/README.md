...
每个 API、Worker 或一次性任务进程在创建子 logger 前调用一次 `configureLogging()`，退出时调用 `shutdownLogging()`。Logger 可注入模块；模块不得重复配置或提前关闭进程级 sink。

## 配置与运行资源

输出级别、pretty 格式和可选 JSONL 文件由各进程配置决定。开发日志流是有界内存队列，随进程退出释放；文件路径和 sink 也由进程组合根创建及关闭。本包不连接数据库或 Collector。
