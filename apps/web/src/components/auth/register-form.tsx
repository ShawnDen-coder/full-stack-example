...
export function RegisterForm({
  ...
  const fields = [
    ...
    form.register("confirmation", {
      required: "请确认密码",
      validate: (v) => v === form.getValues("password") || "两次输入的密码不一致。",
    }),
  ];
  const defs = [
  ...
  return (
    ...
    >
      <Stack spacing={2.5}>
        {defs.map((d, i) => {
          const reg = fields[i]!;
          const key = ["name", "email", "password", "confirmation"][i] as keyof RegisterValues;
          return (
            <TextField
              key={key}
              {...reg}
              ...
              label={d.label}
              error={Boolean(e[key])}
              helperText={e[key]?.message as string | undefined}
              fullWidth
            />
          );
        })}
