...
登录后先通过 Auth Client 获取 Session 和组织列表，再设置活动组织，最后加载租户业务数据。页面请求收到 `401` 时进入登录流程，收到 `400` 时提示选择组织，收到 `403` 时显示权限不足，不能把这些状态统一当作普通网络错误。
